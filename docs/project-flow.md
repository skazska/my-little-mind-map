...
- sprint/phase: is transition between consistent states of the product, consisting of a set of tasks that can be defined with clear requirements, design, acceptance criteria, deliverables with reasonable complexity.

## Flow structure

1. [PLAN](../project/PLAN.md) - high-level plan and roadmap, milestones (like POC, POC3, MVP etc.).
2. Each milestone consists of sprints/phases and represented by file `project/{MILESTONE}.md` containing overall requirements, decisions, status and `results and learnings`, and a folder `project/{MILESTONE}/` with optioonal:
...
3. Each phase/sprint consists of tasks and represented by file `project/{MILESTONE}/{MILESTONE}-sprint-{NUM}.md` with overall requirements, decisiond and status and a folder `project/{MILESTONE}/{MILESTONE}-sprint-{NUM}/` with optional:
...
4. Each task represented by file `project/{MILESTONE}/{MILESTONE}-sprint-{NUM}/{TASK_NUM}_{TASK_NAME}.md` with details of the task, including goal, requirements, design, deliverables, acceptance criteria, etc. and optionally a folder `project/{MILESTONE}/{MILESTONE}-sprint-{NUM}/{TASK_NUM}_{TASK_NAME}/` with additional artifacts or implementation details.
...
6. All tasks produce changes in repository which should commited, pushed and merged via PR according to [git flow guidelines](../git-flow.md). Task file and folder should be updated with links to PR, commits, and other relevant artifacts.
